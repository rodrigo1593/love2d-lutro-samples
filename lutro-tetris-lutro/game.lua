...
local function setMatrixCells(matrix, width, height, value)
	for i = 0, width - 1 do
        matrix[i] = {}
		for j = 0, height - 1 do
			matrix[i][j] = value
...
local function setTetromino(indexTetromino, tetromino)
	-- Initialize tetromino cells to empty cells.
	setMatrixCells(tetromino.cells, Game.TETROMINO_SIZE, Game.TETROMINO_SIZE, Game.Cell.EMPTY)
...
local function isFlagSet(set, flag)
...
function Game:start()
	...
	setMatrixCells(self.m_map, Game.BOARD_TILEMAP_WIDTH, Game.BOARD_TILEMAP_HEIGHT, Game.Cell.EMPTY)

	-- Initialize falling tetromino.
	setTetromino(Platform:random() % TETROMINO_TYPES, self.m_fallingBlock)
	...
	setTetromino(Platform.random() % TETROMINO_TYPES, self.m_nextBlock)
...
function Game:rotateTetromino(clockwise)
	...
	setMatrixCells(rotated, Game.TETROMINO_SIZE, Game.TETROMINO_SIZE, Game.Cell.EMPTY)
...
function Game:update()
    local JOY_LEFT = love.keyboard.isDown("left")--love.joystick.isDown(1, RETRO_DEVICE_ID_JOYPAD_LEFT)
    local JOY_RIGHT = love.keyboard.isDown("right")--love.joystick.isDown(1, RETRO_DEVICE_ID_JOYPAD_RIGHT)
    local JOY_DOWN = love.keyboard.isDown("down")--love.joystick.isDown(1, RETRO_DEVICE_ID_JOYPAD_DOWN)
    local JOY_UP = love.keyboard.isDown("up")--love.joystick.isDown(1, RETRO_DEVICE_ID_JOYPAD_UP)
    local JOY_A = love.keyboard.isDown("x")--love.joystick.isDown(1, RETRO_DEVICE_ID_JOYPAD_A)
    local JOY_B = love.keyboard.isDown("z")--love.joystick.isDown(1, RETRO_DEVICE_ID_JOYPAD_B)
...
function Game:onEventStart(command)
	if (command == Game.Event.QUIT) then
        self.m_errorCode = Game.Error.PLAYER_QUITS
